...
if ($id_assistente == 1) {
    // Incluindo o id na consulta para administradores
    $sql = "SELECT id, nome, local, funcao, cress, atendimentos, matricula FROM assistentes";
    $stmt = $db->prepare($sql);
} else {
    ...
    if ($stmtSetor->rowCount() > 0) {
        ...
        // Incluindo o id na consulta para assistentes
