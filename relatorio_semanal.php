...
if ($id_assistente == 1) {
    $sql = "SELECT a.protocolo, a.assunto, a.data, a.descricao, u.nome AS nome_usuario, s.nome AS nome_assistente 
            ...
            WHERE a.data BETWEEN :inicio_semana AND :fim_semana
            ORDER BY a.data ASC";
} else {
    $sql = "SELECT a.protocolo, a.assunto, a.data, a.descricao, u.nome AS nome_usuario, s.nome AS nome_assistente 
            FROM atendimentos a
...
$stmt->bindParam(':fim_semana', $fim_semana);
if ($id_assistente != 1) {
...
if ($data) {
    ...
    $conteudo_pdf .= "<title>Relatório Semanal</title>";
    $conteudo_pdf .= "<style>
        body { font-family: Arial, sans-serif; color: #333; }
        .header { text-align: center; margin-bottom: 20px; }
        .logo { width: 150px; }
        h1 { font-size: 20px; color: #6f42c1; text-align: center; }
        .table { width: 100%; border-collapse: collapse; margin-top: 20px; }
        .table th, .table td { border: 1px solid #ddd; padding: 8px; text-align: left; }
        .table th { background-color: #f2f2f2; color: #6f42c1; }
    ...
    $conteudo_pdf .= "<div class='header'>
                        ...
                        <h1>Relatório Semanal de Atendimentos</h1>
                      </div>";
    ...
    $dompdf->stream("Relatorio_Semanal_" . date('Y-m-d') . ".pdf");
} else {
    $dompdf = new Dompdf();
    $dompdf->loadHtml("<h3>Nenhum atendimento lançado esta semana</h3>");
